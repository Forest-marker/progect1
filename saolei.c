...
//2.某个位置当前是翻开还是未翻开 show_map
//如果翻开，当前周围有几个雷，0-8表示周围有几个雷
#define M_ROW 9
#define M_COL 9
...
void Update(char mine_map[M_ROW][M_COL], char show_map[M_ROW][M_COL], int row, int col){
	//根据这个位置，判断这个位置周围8个格子中有几个雷
	//就把数字设置到show_map对应的位置中
...
int main(){
	...
	while (1){
		...
		//5.判定游戏是否胜利（翻开所有的不是雷的格子）
		//用count代表已经翻开多少个不是雷的格子
		++count;
